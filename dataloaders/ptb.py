...
class ptb_char():
	...
	def process_one(self, mode, exclude_char, exclude_list=None, char_to_idx=None):		
		...
		print("Generating pickle dump for", mode)
		...
		if char_to_idx is None:
			...
			# replace all unknwon characters with exclude char
			unk_chars = set(corpus) - set(char_to_idx.keys())
			for c in unk_chars:
				corpus = corpus.replace(c, exclude_char)
		...
		return inputs[:2000], labels[:2000], char_to_idx
	...
	def process_all(self):
		if os.path.exists(self.pkl_name):
			...
			final['test'] = self.process_one('test', self.exclude_char, char_to_idx=char_to_idx)
			final['val'] = self.process_one('valid', self.exclude_char, char_to_idx=char_to_idx)
