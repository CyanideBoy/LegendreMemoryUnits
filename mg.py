...
class MGDataset:
    ...
    def __init__(self, path, seq_len, future_predict=15, number_of_series=[2, 2, 2], x0 = None):
        ...
        if x0 is None:
            self.x0 = [0.9697, 0.9699, 0.9794, 1.0003, 
        ...
        self.pkl_name = os.path.join(self.db_path, 'MG-Data')+'/'+str(seq_len)+'_'+ str(future_predict)+'_'+ ('-').join([str(x) for x in number_of_series])+'.pkl'
        print(self.pkl_name)
    ...
    def get_dataset(self):
        ...
        self.y_train = np.zeros([self.number_of_series[0],1,self.future_predict])
        self.x_val = np.zeros([self.number_of_series[1],1,self.seq_len])
        self.y_val = np.zeros([self.number_of_series[1],1,self.future_predict])
        self.x_test = np.zeros([self.number_of_series[2],1,self.seq_len])
        self.y_test = np.zeros([self.number_of_series[2],1,self.future_predict])
        ...
        for i in range(self.number_of_series[0]): 
            ...
            self.y_train[i,0,:] = self.seq_train[i*(self.seq_len+self.future_predict)+self.seq_len:(i+1)*(self.seq_len+self.future_predict)]
        ...
        for i in range(self.number_of_series[1]):
            ...
            self.y_val[i,0,:] = self.seq_val[i*(self.seq_len+self.future_predict)+self.seq_len:(i+1)*(self.seq_len+self.future_predict)]
        ...
        for i in range(self.number_of_series[2]): 
            ...
            self.y_test[i,0,:] = self.seq_test[i*(self.seq_len+self.future_predict)+self.seq_len:(i+1)*(self.seq_len+self.future_predict)]
...
if __name__ == '__main__':
    a = MGDataset('.', 200, 15)
    data = a.get_dataset()
    plt.plot(list(range(200)),data['train'][0][0,0,:200], c='r')
    plt.plot(list(range(200)),data['val'][0][0,0,:200], c='b')
    plt.plot(list(range(200,215)),data['val'][1][0,0,:], c='black')
    plt.show()